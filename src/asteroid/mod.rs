...
pub fn update_asteroid_velocity(
    mut query: Query<(&Asteroid, &Transform, &mut Acceleration)>,
    player_query: Query<&Transform, With<Player>>,
) {
    let player_transform = player_query.single();
    for (asteroid, transform, mut acceleration) in query.iter_mut() {
        if let Asteroid::Attached = asteroid {
            let mut direction = (player_transform.translation - transform.translation).truncate();
            acceleration.0 = direction * ASTEROID_GRAVITY_SCALE;
            direction = direction.normalize_or_zero();
            acceleration.0 -= direction * ASTEROID_REPULSION_SCALE;
        }
    }
}

pub fn update_asteroid_state(
    mut asteroid_query: Query<(&mut Asteroid, &Transform, &mut Velocity, &mut Acceleration, &mut Drag)>,
    player_query: Query<&Transform, With<Player>>,
    input: Res<GameInput>,
) {
    let player_transform = player_query.single();
    let just_released = input.just_released;
    for (mut asteroid, transform, mut velocity, mut acceleration, mut drag) in asteroid_query.iter_mut() {
        if let Asteroid::Attached = *asteroid {
            if just_released {
